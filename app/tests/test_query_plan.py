            ...
            parse_plan(
                {
                    ...
                    "intent_reason": "recipe request",
                    "search_query": "x",
                    "requirements": [{"field": "color", "op": "eq", "value": "red"}],
                }
            )
...
class TestEvaluation:
    ...
    def test_ingredients_contains_substring_match(self):
        ...
        r = Requirement("ingredients", "contains", "salt")
        assert (
            evaluate_requirement(r, rec(ingredients=["sea salt flakes", "water"]))
            is True
        )
    ...
    def test_cuisine_eq_exact_after_normalization(self):
        assert (
            evaluate_requirement(Requirement("cuisine", "eq", "Ukrainian"), rec())
            is True
        )
        assert (
            evaluate_requirement(Requirement("cuisine", "eq", "Indian"), rec()) is False
        )
    ...
    def test_time_lte(self):
        assert (
            evaluate_requirement(Requirement("time_minutes", "lte", 75), rec()) is True
        )
        assert (
            evaluate_requirement(Requirement("time_minutes", "lte", 30), rec()) is False
        )

    def test_time_gte(self):
        assert (
            evaluate_requirement(Requirement("time_minutes", "gte", 80), rec()) is False
        )
    ...
    def test_servings_gte_with_unknown(self):
        ...
        assert (
            evaluate_requirement(Requirement("servings", "gte", 4), rec(servings=None))
            is False
        )

    def test_title_contains_case_insensitive(self):
        assert (
            evaluate_requirement(Requirement("title", "contains", "BORSCHT"), rec())
            is True
        )
        assert (
            evaluate_requirement(Requirement("title", "contains", "katsu"), rec())
            is False
        )
